...
def selectTrainTestVal(data, labels, pt, pv):
    ...
    for i in range(1, c+1):
        ...
        num = len(loc1)
        order = np.random.permutation(range(num))#随机打乱数组顺序，返回一个新数组，并不改变原数组。
                                                 #np.random.shuffle会直接在原数组上打乱
        loc1 = loc1[order]  #在1：num的序列上进行顺序打乱，并不影响坐标的匹配问题
        loc2 = loc2[order]

        num1 = int(np.round(num*(pt)))  # 训练
        num2 = int(np.round(num*(pt+pv)))  # 验证
        # print(num1,num2,num)
        x = np.vstack([x, data[loc1[:num1], loc2[:num1], :]])  #vstack 沿行（v=vertical垂直）堆叠
                                                               #hstack 沿列（h=horizontal水平）堆叠
                                                               #stack 沿指定轴进行堆叠，axis=0,...需再进行验证
    ...
    return x, xb, x_loc, y, yb, y_loc,z,zb,z_loc  #xb,yb,zb是每一类的标签
...
def selectUlabelSamples(data, labels, pu):
    ...
    num = len(loc1)
    print(num)
    order = np.random.permutation(range(num))#np.random.permutation随机排列一个数组
    loc1 = loc1[order]
    loc2 = loc2[order]
...
#    data_name = 'PaviaU' 
    ...
    if data_name == 'Indian_pines':
        pt,pv,pu = 0.05,0.05,0.0 #Train Val Ulabel S
    elif data_name == 'PaviaU':
        pt,pv,pu = 0.03,0.03,0
    elif data_name == 'KSC':
    ...
    sio.savemat(path+'/data/'+data_name+'/'+data_name+'_pre.mat', {'train_x':train_x,
                ...
                'labels_ori':labels_ori})#sio.savemat可将一个行数组存储成二维的，np.save不能。
    print("Finished!")
